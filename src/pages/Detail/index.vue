      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model.number="curNum" @change="changeCurNum">
                <a href="javascript:" class="plus" @click="curNum++">+</a>
                <a href="javascript:" class="mins" @click="curNum>1?curNum--:curNum=1">-</a>
              </div>
              <div class="add" @click="goShoppCar">
                <a>加入购物车</a>
              </div>
            </div>
          </div>
...
<script>
  ...
  export default {
    ...
    mounted(){
      this.$store.dispatch('getDetailsList',this.$route.params.detailId)
      // console.log(this.$store.getters);s
    },
    computed:{
    ...
    methods:{
      ...
      // 加入购物车
      async goShoppCar(){
        // 这里不用存储数据，但是要知道结果是成功还是失败，成功就跳转路由，失败就弹出原因
        try{
          await this.$store.dispatch('getCarData',{skuId:this.$route.params.detailId,skuNum:this.curNum})
          // 只传地买的数量商品信息的话存到会话存储里面，不能直接接存贮对象，要先转成json'字符串在存进去
          sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
          this.$router.push({name:'AddCartSuccess',query:{skuNum:this.curNum}})
        }catch(error){
          alert(error.message)
        }
